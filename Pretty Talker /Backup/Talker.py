...
class Controller(object):
  ...
  def peerLoop(self):
      ...
      while self.peer_running and self.running:
        try:
  ...
  def createKey(self,args):
    if not args:
      self.talk_print.error("createKey < useranme > ..")
      return
  ...
  def run(self):
    ...
    if args[1] == 'start':
      self.start(args[2:])
    elif args[1] ==  'createKey':
      self.createKey(args[2:])
      return
    else:
      self.talk_print.error("Please Use Command: < start > | < createKey >")
      return

    self.talk_print.text('service started,please enter commands: [conn/!close/!quit]')
    while self.running:
      self.talk_print.prompt()
      text = raw_input()
      fps = text.strip().split()
      if not fps:
        continue
      if fps[0].lower() == 'conn' and self.sock_peer == None:
        self.doCommandConnect(fps[1:])
        continue
      ...
      if fps[0] == '!close' and self.sock_peer:
        self.doCommandClose()
        continue
